...
df = pd.read_csv('Salesforce_Stock/Salesforce (CRM) From 2004 To Dec-2024.csv', delimiter=',')
# this file has mixed timezones in the date column, utc=True fixes it
...
summary_stats.to_csv(f"{OUTPUT_DIR}/CRM_summary_stats.csv", index=False)
weekday_avg.to_csv(f"{OUTPUT_DIR}/CRM_weekday_avg_return.csv")
month_avg.to_csv(f"{OUTPUT_DIR}/CRM_month_avg_return.csv")
importance.to_csv(f"{OUTPUT_DIR}/CRM_feature_importance.csv")
anomaly_table.to_csv(f"{OUTPUT_DIR}/CRM_anomaly_days.csv", index=False)

fig = plt.figure(figsize=(20, 22))
fig.suptitle("Salesforce (CRM) - Full Research Summary", fontsize=18, fontweight="bold")
...
plt.savefig(f"{OUTPUT_DIR}/CRM_all_charts.png", dpi=120)
plt.show()
