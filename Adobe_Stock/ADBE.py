...
df = pd.read_csv('Adobe_Stock/Adobe.csv', delimiter=',')
...
# adobe's ipo week rows have Open = 0, the cleaning step below drops these
...
summary_stats.to_csv(f"{OUTPUT_DIR}/ADBE_summary_stats.csv", index=False)
weekday_avg.to_csv(f"{OUTPUT_DIR}/ADBE_weekday_avg_return.csv")
month_avg.to_csv(f"{OUTPUT_DIR}/ADBE_month_avg_return.csv")
importance.to_csv(f"{OUTPUT_DIR}/ADBE_feature_importance.csv")
anomaly_table.to_csv(f"{OUTPUT_DIR}/ADBE_anomaly_days.csv", index=False)

fig = plt.figure(figsize=(20, 22))
fig.suptitle("Adobe (ADBE) - Full Research Summary", fontsize=18, fontweight="bold")
...
plt.savefig(f"{OUTPUT_DIR}/ADBE_all_charts.png", dpi=120)
plt.show()
